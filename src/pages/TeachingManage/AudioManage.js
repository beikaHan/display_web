...
export default class VideoManage extends Component {
  ...
  render() {
    const {teachingManage: {loading: ruleLoading, audioData, audioItem, classifyAllData}, form: {getFieldDecorator}} = this.props;
          ...
          <Form layout="inline">
            ...
            <FormItem label="分类" {...formItemLayout}>
              {getFieldDecorator('classifyId', {
                  initialValue: addModalType === 'edit' && audioItem && audioItem.classifyId ? audioItem.classifyId : classifyAllData && classifyAllData[0] ? classifyAllData[0].id : '',
                },
              )(
            ...
            <FormItem label="音频上传" {...formItemLayout}>
              {getFieldDecorator('resourceId', {
                  initialValue: addModalType === 'edit' && audioItem && audioItem.resourceId ? audioItem.resourceId : '',
                  rules: [{
                    required: true,
                    message: '音频上传',
                  }],
                ...
                <Upload
                  ...
                  <Button>
                    <Icon
                      type={false? 'loading' : 'upload'}/> {resourceId ? '已有资源' : addModalType === 'edit' && audioItem.resourceId ? '已有资源' : 'Upload'}
                  </Button>
                </Upload>,
              )}
            </FormItem>
            <FormItem label="缩略图" {...formItemLayout}>
              {getFieldDecorator('thumbnailResourceId', {
                  initialValue: addModalType === 'edit' && audioItem && audioItem.thumbnailResourceId ? audioItem.thumbnailResourceId : '',
                  rules: [{
                    required: true,
                    message: '缩略图',
                  }],
                ...
                <Upload
                  ...
                  {imageUrl ? <img src={imageUrl}
                                   alt="avatar"/> : addModalType === 'edit' && audioItem && audioItem.thumbnailUrl ?
                    <img src={audioItem.thumbnailUrl} alt="avatar"/> : <div>
