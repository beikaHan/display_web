...
class DishEdit extends Component {
  ...
  renderForm() {
    const {itemId, addVisible, itemDetails, schoolQuestionTopicData, items, addModalTitle, recourseList, titleAll, onUploadChange} = this.props;
            ...
            <Form onSubmit={this.handleChange} layout="inline">
              ...
              <FormItem label="描述" {...formItemLayout} colon={false}>
                {getFieldDecorator('content', {
                  ...
                  ], initialValue: itemId != undefined && itemDetails.content ? itemDetails.content : '',
                })(
                  <TextArea
                    placeholder={itemId != undefined && itemDetails.content != undefined && itemDetails.content.length > 0 ? itemDetails.content : '请输入描述'}
              ...
              <FormItem label="触发方式" {...formItemLayout} colon={false}>
                {getFieldDecorator('targetType', {
                  initialValue: itemId != undefined && itemDetails.targetType ? itemDetails.targetType : 1,

                })(
                  <Select placeholder={1} getPopupContainer={() => document.getElementById('targetType')}>
                    <Option value={1} key={1}>默认</Option>
                    <Option value={2} key={2}>头衔触发</Option>
                    <Option value={3} key={3}>分数触发</Option>
                    <Option value={4} key={4}>进入展厅</Option>
                  </Select>,
                )}
              </FormItem>
              {getFieldValue('targetType') == 3 ? <FormItem label="分数" {...formItemLayout} colon={false}>
                {getFieldDecorator('targetScore', {
                  initialValue: itemId != undefined && itemDetails.targetScore ? itemDetails.targetScore : 0,
                  rules: [
                    {required: true, message: '请输入分数'},
                  ],
                })(
                  <InputNumber min={1} style={{width: '100%'}} placeholder={'请输入分数'} autoComplete="off"/>,
                )}
              </FormItem> : null}
              {getFieldValue('targetType') == 2 ? <FormItem label="头衔" {...formItemLayout} colon={false}>
                {getFieldDecorator('rankId', {
                  initialValue: itemId != undefined && itemDetails.rankId ? itemDetails.rankId : '',
                })(
                  <Select placeholder={1} getPopupContainer={() => document.getElementById('rankId')}>
                    {titleObj}
                  </Select>,
                )}
              </FormItem> : null}
