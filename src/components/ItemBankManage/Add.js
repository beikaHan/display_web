...
class DishEdit extends Component {
  ...
  renderForm() {
    ...
    schoolQuestionTopicData &&
      schoolQuestionTopicData.map(el => {
            ...
            <Form onSubmit={this.handleChange} layout="inline">
              <FormItem label="知识点" {...formItemLayout} colon={false}>
                {getFieldDecorator('questionTopicId', {
                  initialValue:
                    ...
                    itemDetails.questionTopicId != undefined
                  ...
                  <Select
                    placeholder={'无'}
                    getPopupContainer={() => document.getElementById('questionTopicId')}
                  >
                    {questionTopicObj}
                  </Select>
                )}
              ...
              <FormItem label="难易度" {...formItemLayout} colon={false}>
                {getFieldDecorator('level', {
                  initialValue:
                    itemId != '' && itemDetails != undefined && itemDetails.level != undefined
                      ? itemDetails.level
                      : 1,
                  rules: [
                    {
              ...
              <FormItem label="问题类型" {...formItemLayout} colon={false}>
                {getFieldDecorator('questionType', {
                  initialValue:
                    itemId != '' &&
                    itemDetails != undefined &&
                    itemDetails.questionType != undefined
                      ? itemDetails.questionType
                      : 1,
                  rules: [
                    {
                      required: true,
                  ...
                  <Select
                    placeholder={1}
                    getPopupContainer={() => document.getElementById('questionType')}
                  >
                    <Option value={1} key={1}>
                      单选
                    </Option>
              ...
              <FormItem label="题目信息" {...formItemLayout} colon={false}>
                {getFieldDecorator('content', {
                  ...
                  initialValue:
                    itemId != '' && itemDetails != undefined && itemDetails.content != undefined
                      ? itemDetails.content
                      : '',
                })(
                  <TextArea
                          ...
                          return (
                            <div className={styles.upload} key={el.key}>
                              ...
                              <Button
                                ...
                                type="danger"
                                ghost
                                onClick={() => this.changeUploadList('minus', el.key, el.sort)}
                              >
                                删除
                              </Button>
                            </div>
                    ...
                    <Checkbox.Group style={{ width: '100%' }}>
                      {items &&
                        items.map((el, idx) => {
                          return (
                            <div className={styles.upload} key={el.key}>
                              <Checkbox value={idx + 1} key={idx + 1}>
                                {idx + 1}
                              </Checkbox>
                              <Input
                                defaultValue={el.content}
                              ...
                              <Button
                                ...
                                type="danger"
                                ghost
                                onClick={() => this.changeUploadList('minus', el.key)}
                              >
                                删除
                              </Button>
                            </div>
                ...
                {getFieldDecorator('materialResourceId')(
                  <div className={styles.img} onClick={this.showPicImportModal}>
                    ...
                    >
                      {imageUrl ? (
                      ...
                      ) : itemId != '' && itemDetails != undefined && itemDetails.materialUrl ? (
                        itemDetails.materialType == 'jpg' ||
                        itemDetails.materialType == 'png' ||
                        itemDetails.materialType == 'jpeg' ? (
                        ...
                        <div>
                          ...
                          <div>
                            图片仅支持jpg或png格式，图片不得超过2MB，比例为16:9，视频大小不得超过200MB
                          </div>
                        </div>
                      )}
                    </Upload>
              ...
              {getFieldValue(`questionType`) === 4 || items.length >= 9 ? null : (
