              ...
              <FormItem label="分类" {...formItemLayout} colon={false}>
                {getFieldDecorator('classifyId', {
                  initialValue:
                    itemId != undefined &&
                    itemDetails != undefined &&
                    itemDetails.classifyId != undefined
                      ? itemDetails.classifyId
                      : classifyDisplayAll && classifyDisplayAll[0]
                      ? classifyDisplayAll[0].id
                      : '',
                  rules: [
                    {
                      required: true,
                  ...
                  <Select
                    placeholder={'请选择分类'}
                    getPopupContainer={() => document.getElementById('classifyId')}
                  >
                    {classifyObj}
                  </Select>
                )}
                    ...
                    <div className={styles.upload} key={el.key}>
                      <FormItem label="" colon={false}>
                        {getFieldDecorator(`${el.key}${el.resourceId}`, {
                          initialValue:
                            (itemId != undefined && itemDetails != undefined) ||
                            el.type != undefined
                              ? el.type
                              : 1,
                        })(
                      ...
                      <Upload
                        ...
                        onChange={info =>
                          this.handleresourceChange(
                            info,
                            el.key,
                            getFieldValue(`${el.key}${el.resourceId}`)
                          )
                        }
                      >
                        <Button style={{ marginTop: '4px' }}>
                          <Icon type={uploading[el.key] ? 'loading' : 'upload'} />{' '}
                          {el.resourceId ? '已有资源' : 'Upload'}
                        </Button>
                      </Upload>
              ...
              <FormItem label="" colon={false}>
                {getFieldDecorator('thumbnailResourceId', {
                  initialValue:
                    itemId != undefined &&
                    itemDetails != undefined &&
                    itemDetails.thumbnailResourceId
                      ? itemDetails.thumbnailResourceId
                      : '',
                })(
                  <div className={styles.img} onClick={this.showPicImportModal}>
                    ...
                    >
                      {imageUrl ? (
                        ...
                        <img src={itemDetails.thumbnailUrl} alt="avatar" />
                      ) : (
                        <div>
